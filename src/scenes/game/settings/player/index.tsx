...
import {isPool15OnlyGame, isPoolGame, isSnookerGame} from 'utils/game';
import {
  PLAYER_NUMBER,
  PLAYER_NUMBER_POOL,
...
interface Props {
  ...
  onSelectSnookerSetTarget?: (addedPoint: number, index: number) => void;
...
const PlayerSettingsComponent = ({
  ...
  const isSnooker = useMemo(() => isSnookerGame(category), [category]);
  const usePoolLikeSkin = isPool || isSnooker;
  ...
  const playerNumberOptions = useMemo(() => {
    if (isSnooker || isPool15OnlyGame(category) || (isPool && gameMode === 'pro')) {
  ...
  }, [category, gameMode, isPool, isSnooker]);
  ...
  const renderSnookerSetTarget = useCallback(() => {
    if (!isSnooker) {
      return null;
    }
    ...
    const steps = [-25, -20, -15, -10, -5, currentTarget, 5, 10, 15, 20, 25];
    const targetMap = steps.reduce((acc, item, index) => {
      acc[`snooker-set-target-${index}`] = item;
      return acc;
    }, {} as Record<string, number>);

    return renderSelectorRow(
      isEnglish ? 'Frame points' : 'Điểm set',
      targetMap,
      currentTarget,
      onSelectSnookerSetTarget || (() => {}),
      true,
  ...
  const renderPlayerItem = useCallback(
    (player: Player, index: number) => {
      ...
      const isClassicDarkCard = !usePoolLikeSkin && index >= 2;
      const avatarSize = usePoolLikeSkin ? adaptive.s(48) : adaptive.s(44);
      const flagWidth = usePoolLikeSkin ? adaptive.s(36) : adaptive.s(34);
      const flagHeight = usePoolLikeSkin ? adaptive.s(24) : adaptive.s(22);
      ...
      return (
        <View
          ...
          style={[
            styles.playerCard,
            usePoolLikeSkin ? styles.playerCardPool : {backgroundColor: currentPlayer.color},
          ]}>
          <Pressable
          ...
          <View style={styles.playerCardRight}>
            <View style={styles.playerCardTop}>
              <EditablePlayerNameInput
                value={playerName}
                index={index}
                isPool={usePoolLikeSkin}
                inputStyle={[styles.nameInput, usePoolLikeSkin && styles.nameInputPool]}
            ...
            <View style={[styles.scoreRow, usePoolLikeSkin && styles.scoreRowPool]}>
                ...
                return (
                  <Pressable
                    ...
                    style={({pressed}) => [
                      ...
                      usePoolLikeSkin && styles.scoreItemPool,
                      isCenter && usePoolLikeSkin && styles.scoreItemCenterPool,
                    ...
                    <Text
                      style={[
                        ...
                        usePoolLikeSkin && styles.scoreTextPool,
                      ]}>

...
import {listUsbDevices} from 'services/uvc';
...
const debugCameraSourceLog = (message: string, extra?: Record<string, any>) => {
  recordDebugLog('VideoRecording', message, extra);
};

const hasUsbVideoDeviceNow = async (): Promise<boolean> => {
  ...
  try {
    ...
    const hasVideo = devices.some(device => device.looksLikeVideo);
    debugCameraSourceLog('[USBCamera] usb-devices', {
      reason: 'webcam-viewmodel-source-select',
      devices,
      hasVideo,
    });
    return hasVideo;
  } catch (error) {
    debugCameraSourceLog('[USBCamera] reconnect-failed', {
      reason: 'webcam-viewmodel-usb-probe-error',
...
const getAvailableCameraSources = (): CameraSource[] => {
  ...
  // Android offline bản này ưu tiên USB webcam. Khi đã thấy UVC thì nút Đổi cam
  // không vòng về camera điện thoại nữa.
  if (Platform.OS === 'android' && hasUvc) {
    return ['external'];
  }
  ...
  return hasUvc ? ['external'] : ['back', 'front'];
};
...
const WebCamViewModel = (props: Props) => {
  ...
  useEffect(() => {
    ...
    const initCameraSource = async () => {
      ...
      try {
        const hasUsb = await hasUsbVideoDeviceNow();
        if (mounted && hasUsb) {
          debugCameraSourceLog('[CameraSource]', {
            selectedMode: 'usb',
            lockedMode: 'usb',
            reason: 'usb-video-device-present-before-ip-config',
          });
          debugCameraSourceLog('[CameraSource] fallback-blocked', {
            from: 'usb',
            to: 'ip',
            reason: 'user-selected-usb',
          });
        ...
        const ipCameraConfig = await loadIpCameraConfig();

        if (mounted && isIpCameraConfigured(ipCameraConfig)) {
          const rtspCandidates = buildRtspCameraCandidates(ipCameraConfig);
          const rtspUrl = rtspCandidates[0] || buildRtspCameraUrl(ipCameraConfig);

          debugCameraSourceLog('[CameraSource]', {
            selectedMode: 'ip',
            lockedMode: 'ip',
            sourceUri: rtspUrl.replace(/rtsp:\/\/([^:]+):([^@]+)@/i, 'rtsp://$1:***@'),
            candidateCount: rtspCandidates.length,
          });
          console.log('[WebCam] IP camera configured, preparing RTSP candidates', {
            ipAddress: ipCameraConfig.ipAddress,
            candidateCount: rtspCandidates.length,
          });

          setWebcam({
            webcamIP: ipCameraConfig.ipAddress || rtspUrl,
            ...
            rtspUrl,
            rtspCandidates,
            cameraName: ipCameraConfig.name,
          });
          ...
          props.setIsCameraReady(false);
          await AsyncStorage.setItem(keys.WEBCAM_TYPE, WebcamType.webcam);
          return;
        }
      } catch (error) {
        console.log('[WebCam] IP camera config load failed:', error);
      }
      ...
      (globalThis as any).__APLUS_SELECTED_CAMERA_MODE__ = 'phone';
      await AsyncStorage.setItem(keys.WEBCAM_TYPE, WebcamType.camera);
      getCameraData();
    };
  ...
  }, [clearPlaybackTimers, getCameraData, resetConnectionState, props.setIsCameraReady]);
    ...
    requestReadWriteStorage().then(async isGranted => {
      ...
      const sourceUrl =
        webcamType === WebcamType.webcam && webcam
          ? webcam.rtspUrl || `${WEBCAM_HOST}${webcam.username}:${webcam.password}@${webcam.webcamIP}:${WEBCAM_PORT}${WEBCAM_PATH}`
          : undefined;
  ...
  const onSwitchCamera = useCallback(async () => {
    ...
    if (webcamType === WebcamType.camera) {
      ...
      (globalThis as any).__APLUS_SELECTED_CAMERA_MODE__ = nextSource === 'external' ? 'usb' : 'phone';
    ...
    // Android IP camera/RTSP is configured from Settings. Do not let the
    // Đổi cam button fall back to phone camera, because that makes the preview
    // flicker between RTSP and the built-in camera. For IP camera, this button
    // only refreshes the current RTSP stream.
    if (webcamType === WebcamType.webcam && webcam?.rtspUrl) {
      props.setIsCameraReady(false);
      setRefreshing(true);
      (globalThis as any).__APLUS_SELECTED_CAMERA_MODE__ = 'ip';
      setCurrentCameraSourceSnapshot('external');
      setUrl(webcam.rtspUrl);
      setAutoConnect(true);
      setIsWebcamStarted(true);
    ...
    () => ({
      ...
      source:
        webcamType === WebcamType.webcam
          ? ({uri: url, type: 'rtsp', rtspCandidates: webcam?.rtspCandidates || (url ? [url] : [])} as any)
          : {uri: url, type: 'mov'},
      onRefresh,
      onSwitchCamera,
      onDelay,

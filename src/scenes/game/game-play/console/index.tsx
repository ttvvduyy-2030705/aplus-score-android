...
const QuadButton = ({
  ...
  labels: string[];
  icons?: Array<number | undefined>;
  onPresses?: Array<(() => void) | undefined>;
  tones?: ActionButtonTone[];
  compact?: boolean;
  extraCompact?: boolean;
  poolCompact?: boolean;
  tight?: boolean;
  ...
  const textStyle = [
    styles.quadButtonText,
    poolCompact ? styles.poolQuadButtonText : undefined,
    compact ? styles.compactQuadButtonText : undefined,
    extraCompact ? styles.extraCompactQuadButtonText : undefined,
    tight ? styles.tightQuadButtonText : undefined,
  ];
  const iconStyle = [
    styles.quadButtonIcon,
    poolCompact ? styles.poolQuadButtonIcon : undefined,
    compact ? styles.compactActionButtonIcon : undefined,
    extraCompact ? styles.extraCompactActionButtonIcon : undefined,
    tight ? styles.tightActionButtonIcon : undefined,
  ];
  ...
  return (
    <View
      direction={'row'}
      style={[
        styles.quadButtonRow,
        compact ? styles.compactQuadButtonRow : undefined,
      ]}>
      {labels.map((label, index) => (
        <Button
          key={`${label}-${index}`}
          onPress={onPresses?.[index]}
          style={[
            styles.quadButton,
            poolCompact ? styles.poolQuadButton : undefined,
            compact ? styles.compactQuadButton : undefined,
            extraCompact ? styles.extraCompactQuadButton : undefined,
            tight ? styles.tightQuadButton : undefined,
            buttonToneStyle(tones?.[index] || 'dark'),
            buttonHeightStyle,
          ]}>
          <ActionButtonContent
            label={label}
...
const GameConsole = (props: ConsoleViewModelProps) => {
  ...
  const targetLabel = tr('Mục tiêu', 'Goal');
  const totalTimeText = viewModel.displayTotalTime();
  const players = props.playerSettings?.playingPlayers || [];
  const snookerSetScore1 = Math.max(
    0,
    Math.round(
      Number(
        (players[0] as any)?.snooker?.setScore ??
          (players[0] as any)?.setScore ??
          (players[0] as any)?.frameScore ??
          0,
      ),
    ),
  );
  const snookerSetScore2 = Math.max(
    0,
    Math.round(
      Number(
        (players[1] as any)?.snooker?.setScore ??
          (players[1] as any)?.setScore ??
          (players[1] as any)?.frameScore ??
          0,
      ),
    ),
  );
  const snookerSetPointTargetRaw = Number(
    (props.gameSettings as any)?.players?.goal?.snookerSetTarget ??
      (props.gameSettings as any)?.players?.goal?.framePointTarget ??
      (props.playerSettings as any)?.goal?.snookerSetTarget ??
      (props.playerSettings as any)?.goal?.framePointTarget ??
      75,
  );
  const snookerSetPointTarget =
    Number.isFinite(snookerSetPointTargetRaw) && snookerSetPointTargetRaw > 0
      ? Math.round(snookerSetPointTargetRaw)
      : 75;
  const snookerTargetSetRaw = Number(props.goal || 0);
  const snookerTargetSet =
    Number.isFinite(snookerTargetSetRaw) && snookerTargetSetRaw > 0
      ? Math.round(snookerTargetSetRaw)
      : 0;
  const hasSnookerMatchWinner =
    isSnooker &&
    snookerTargetSet > 0 &&
    (snookerSetScore1 >= snookerTargetSet ||
      snookerSetScore2 >= snookerTargetSet);
  const targetDisplayValue = isSnooker
    ? `${props.goal} ${tr('set', 'sets')}`
    : props.goal;
  ...
  const startLabel = useMemo(() => {
    ...
    if (props.isPaused) {
      if (isFastMode || isQuickMatchMode) {
        return tr('Tiếp tục', 'Resume');
      }

      return tr(
        isCarom ? 'Bắt đầu' : 'Tiếp tục',
        isCarom ? 'Start' : 'Resume',
      );
    }

    return tr('Tạm dừng', 'Pause');
    ...
    if (isSnooker) {
      ...
      return (
        <TripleButton
          ...
          centerLabel={`↔ ${tr('Đổi lượt', 'Switch turn')}`}
          rightLabel={`✚ ${tr('Thêm giờ', 'Extension')}`}
          onLeftPress={props.onResetTurn}
          onCenterPress={() => props.onEndTurn?.()}
          onRightPress={viewModel.onPressGiveMoreTime}
          leftTone={'green'}
          centerTone={'amber'}
          rightTone={'muted'}
          compact={useResponsiveCompact}
    ...
    if (isSnooker) {
      return (
        <DualButton
          leftLabel={startLabel}
          leftIcon={props.isStarted && !props.isPaused ? undefined : images.game.start}
          rightLabel={hasSnookerMatchWinner ? tr('Kết thúc', 'End') : tr('Kết thúc set', 'End set')}
          rightIcon={images.game.endMatch}
          onLeftPress={handleBottomLeft}
          onRightPress={
            hasSnookerMatchWinner ? viewModel.onStop : props.onEndSnookerSet
          }
          leftTone={'amber'}
          rightTone={'red'}
          compact={useResponsiveCompact}
      ...
      {isPool15 ? (
        ...
        <View style={[styles.topButtonRowWrap, isCameraFullscreen ? styles.hiddenWhenFullscreen : undefined]}>
          <TripleButton
            ...
            rightLabel={`${targetLabel} ${props.goal}`}
...
const createStyles = (adaptive: any, design: any, rules: any) => {
  ...
  snookerControlsCardCompact: {
    borderRadius: 12,
    paddingHorizontal: 8,
    paddingVertical: 6,
    gap: 5,
  },
  snookerControlsHeaderRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    gap: 8,
  },
  snookerControlsTitle: {
    color: '#FFFFFF',
    fontSize: 18,
    lineHeight: 22,
    fontWeight: '900',
  ...
  snookerPointButtonCompact: {
    minWidth: 72,
    minHeight: 32,
    paddingHorizontal: 8,
    paddingVertical: 4,
  },
  snookerPointButtonText: {
    color: '#FFFFFF',
    fontSize: 14,
    lineHeight: 17,
    fontWeight: '900',
  ...
  poolQuadButtonIcon: {
    width: Math.max(14, poolUtilityButtonIconSize - 2),
    height: Math.max(14, poolUtilityButtonIconSize - 2),
  },
  quadButtonText: {
    color: '#FFFFFF',
    fontSize: 16,
  ...
  compactQuadButtonText: {
    fontSize: 13,
    lineHeight: 15,
  },
  extraCompactQuadButtonText: {
    fontSize: 11,
    lineHeight: 13,
  },
  tightQuadButtonText: {
  ...
  poolQuadButtonText: {
    fontSize: Math.max(13, poolUtilityButtonFontSize - 3),
    fontWeight: '900',
    lineHeight: Math.max(15, poolUtilityButtonLineHeight - 3),
    letterSpacing: -0.15,
  },
  disabledButton: {
    opacity: 0.5,

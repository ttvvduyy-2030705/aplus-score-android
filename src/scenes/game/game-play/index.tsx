...
const GamePlay = () => {
  ...
  useEffect(() => {
    RemoteControl.instance.setCurrentScreen?.('gameplay');
    RemoteControl.instance.setEnabled?.(true);

    return () => {
      RemoteControl.instance.setCurrentScreen?.('leaving-gameplay');
      RemoteControl.instance.setEnabled?.(false);
    };
  }, []);

  useEffect(() => {
    RemoteControl.instance.setCurrentScreen?.(
      isCameraFullscreen ? 'gameplay-fullscreen-camera' : 'gameplay',
    );
  }, [isCameraFullscreen]);
  ...
  const useDarkPoolBackground = useMemo(() => {
    return isPoolGame(category) || isSnookerGame(category);
  }, [category]);
  ...
  const renderPlayer = (playerIndex: number) => {
    ...
    return (
      <GamePlayer
        ...
        onSnookerScore={viewModel.onSnookerScore}
        onSnookerFoul={viewModel.onSnookerFoul}
  ...
  const renderConsole = () => {
    return (
      <GameConsole
        ...
        onSnookerScore={viewModel.onSnookerScore}
        onSnookerFoul={viewModel.onSnookerFoul}
        onEndSnookerSet={viewModel.onEndSnookerSet}
  ...
  const renderMainBoard = () => {
    if (isCameraFullscreen) {
      return (
        <View flex={'1'} style={localStyles.fullscreenFill}>
          {renderConsole()}
        </View>
      );
    }
  ...
  return (
    <Container variant="fullscreen" safeAreaDisabled={isCameraFullscreen}>
      <View
        ...
        {!isCameraFullscreen &&
        (isPoolGame(category) || isCaromGame(category) || isSnookerGame(category)) &&
        ...
        {showPauseOverlay ? (
          <View style={styles.warmUpContainer}>
            ...
            <Button
              ...
              <Text color={colors.white} fontSize={warmButtonTextSize}>
                {viewModel.isVarReplayPreparing && !viewModel.isVarReplayReady
                  ? 'Đang chuẩn bị...'
                  : i18n.t('reWatch')}
              </Text>
            </Button>

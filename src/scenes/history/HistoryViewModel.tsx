...
const HistoryViewModel = () => {
  ...
  const refreshScannedHistory = useCallback(async () => {
    ...
    try {
      ...
      recordDebugLog('HistoryScreen', 'scan-failed', {error: String((error as any)?.message || error)});
      setScannedHistoryGames([]);
    }
  }, []);
  ...
  const combinedGames = useMemo(() => {
    ...
    const mergedRealmGames = realmGames.map(game => {
      ...
      if (scannedScoreTotal > realmScoreTotal) {
        recordDebugLog('HistoryFinalize', 'history-list-score-overridden-from-manifest', {
          webcamFolderName,
          realmScoreTotal,
          scannedScoreTotal,
  ...
  const onReWatchGame = useCallback(async (webcamFolderName?: string) => {
    ...
    recordDebugLog('HistoryScreen', 'event', {

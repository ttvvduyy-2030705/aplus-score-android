...
class RemoteControl {
  ...
  private ensureNativeListener = () => {
    ...
    RemoteControlModule.setCurrentScreen?.(this._currentScreen);
  ...
  private onRemoteEvent = (data: RemoteControlKeysNative) => {
    ...
    console.log('[Remote][JS] key received:', {
      currentScreen: this._currentScreen,
      enabled: this._enabled,
      transport: (data as any)?.transport || (data as any)?.source,
      appManagedConnection: (data as any)?.appManagedConnection,
  ...
  public setEnabled = async (enabled: boolean) => {
    ...
    console.log('[Remote][JS] setEnabled input gate only:', {
      enabled,
      currentScreen: this._currentScreen,
      appManagedConnection: false,
      appCalledDisconnect: false,
    });
  ...
  public setCurrentScreen = async (screen: string) => {
    this.ensureNativeListener();
    this._currentScreen = String(screen || 'unknown');
    console.log('[Remote][JS] current screen:', this._currentScreen);
    await RemoteControlModule.setCurrentScreen?.(this._currentScreen);
  };

  public setNewGameHoldRequired = async (required: boolean) => {
    this.ensureNativeListener();
    console.log('[Remote][JS] setNewGameHoldRequired:', {required, currentScreen: this._currentScreen});
    await RemoteControlModule.setNewGameHoldRequired?.(required);
  };

  public scanAndConnect = async () => {
    this.ensureNativeListener();
    console.log('[Remote][JS] scanAndConnect skipped for HID remote. Pair/connect in Android Bluetooth settings.');
  ...
  public disconnect = async () => {
    this._enabled = false;
    console.log('[Remote][JS] disconnect requested: app disables input only, it does not disconnect HID Bluetooth.');

  ...
  useEffect(() => {
    ...
    const statusSubscription = (BLEService as any).addStatusListener?.((data: any) => {
      ...
      if (
        ...
        setIsScanning(false);
        setConnectedDevice({
          id: data?.bluetoothAddress ? String(data.bluetoothAddress) : 'android-hid-remote',
          name: data?.deviceName || 'Bluetooth HID Remote',
          status,
          appManagedConnection: data?.appManagedConnection === true,
        });
      }
